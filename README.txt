...
B1、去除少于在10%细胞中表达的基因+去除表达少于200个基因的细胞，创建seurat对象，计算mt%（线粒体基因表达百分比）
...
B8、注意事项：质控轮数应按实际需求，B6和B7每当有删除细胞操作时都需要再次做聚类分析和find cluster操作（即脚本内选项3操作，选项3操作完才算质控完成1次）

“step A” + “step B”结合可处理多个测序数据，为数据整合前做准备，无需整合的数据可直接跨过“step optional”进行“step C”
...
C1、由使用者重复设置分辨率找出合适的亚群后进行差异基因分析（找markers）
C2、提供seurat所含的多种基因表达图的绘制功能（由使用者选择要画的基因和图类）
C3、细胞注释功能，两种方式可选：由使用者根据基因表达情况手动注释，或者由SingleR自动注释
注释后的数据文件覆盖保存为读取时的文件
###
